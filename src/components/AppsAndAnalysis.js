    ...
    fetch(`https://data-proxy.ebrains.eu/api/v1/buckets/${props.keyValue}?prefix=.nesysWorkflowFiles/alignmentJsons/&delimiter=%2F&limit=50`)
      ...
      .then(data => {
        const items = data.objects.slice(1).map((object, index) => {
          ...
          fetch(`https://data-proxy.ebrains.eu/api/v1/buckets/${props.keyValue}/${object.name}?inline=false&redirect=false&_=${cacheBuster}`)
            .then(response => response.json())
            .then(data => data.url)
            .then(req_url => fetch(req_url))
            .then(resp => resp.json())
            .then(resp => {
              var WebAlign = '✔️';
              var WebWarp = '❌';
              for (let i = 0; i < resp["sections"].length; i++) {
                if (resp["sections"][i].markers) {
                  WebWarp = '✔️';
                  break;
                }
              }
              ...
              fetch(`https://data-proxy.ebrains.eu/api/v1/buckets/${props.keyValue}?prefix=.nesysWorkflowFiles/ilastikOutputs/${nameWithoutExtension}&delimiter=%2F&limit=50`)
                .then(response => response.json())
                .then(data => {
                  var WebIlastik = '❌';
                  console.log("ilastikdata", data)
                  if (data.objects.length > 1) {
                    WebIlastik = '✔️';
                  }
                  fetch(`https://data-proxy.ebrains.eu/api/v1/buckets/${props.keyValue}?prefix=.nesysWorkflowFiles/pointClouds/${nameWithoutExtension}&delimiter=%2F&limit=50`)
                    .then(response => response.json())
                    .then(data => {
                      let NutilWeb = '❌';
                      if (data.objects.length > 1) {
